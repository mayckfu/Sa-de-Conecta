const key = `-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n`;

const cleaned = key
  .replace(/-----(BEGIN|END) [A-Z ]+-----/g, '')
  .replace(/\\n/g, '') // Remove literal \n
  .replace(/\s+/g, '')  // Remove all whitespace
  .replace(/[^A-Za-z0-9+/=]/g, ''); // Safety

console.log('CLEANED_B64_START:' + cleaned.substring(0, 100));
console.log('CLEANED_B64_END:' + cleaned.substring(cleaned.length - 100));
console.log('LENGTH:' + cleaned.length);
console.log('MOD4:' + (cleaned.length % 4));

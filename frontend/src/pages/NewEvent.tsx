...
export const NewEvent: React.FC = () => {
  ...
  const onSubmit = async (data: EventoFormData) => {
    try {
      setIsUploading(true);
      console.log('Iniciando submissão de evento:', data);
      ...
      if (selectedFile) {
        ...
        uploadFormData.append('evento_id', eventData.id);
        ...
        try {
          ...
          console.error('Falha catastrófica ao invocar função:', errorFunc);
          throw errorFunc;
        }
      }
      ...
      await supabase.from('logs_eventos').insert([{
        evento_id: eventData.id,
        acao: 'criacao',
        detalhes: 'Evento registrado inicial na plataforma'
      }]);
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-1">Gestão de Eventos</p>
          <h2 className="font-display text-3xl font-bold text-arc-dark tracking-tight">Criar Evento</h2>
          <p className="text-sm text-slate-400 mt-0.5">Defina informações, logística e materiais em poucos passos.</p>
        </div>
